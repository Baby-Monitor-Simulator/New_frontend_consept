...
function draw() {
  ...
  if (use_stub_data) {
    graphData = buildData(totalDatapoints.value);
  } else {
  ...
  try {
    ...
    if (ChartData.value.length > 0) {
      ...
      for (let i = 0; i < ChartData.value.length; i++) {
        ...
        if (
          ChartData.value[i].total_timesteps >
          totalDatapoints.value - graphTimeStepWidth.value
        ) {
          releventChartData.push(ChartData.value[i]);
        }
      }
      ChartData.value = releventChartData;
      ChartData.value.sort(
  ...
  if (update) {
    ...
    for (let i = 0; i < ChartData.value.length; i++) {
      ...
      if (sectionStart < firstPointInGraph) {
        let releventPointCount =
          dataSection.total_timesteps - firstPointInGraph;
